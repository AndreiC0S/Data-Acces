...
const findAll = (callback) => {
    const queryString = `SELECT id, prenume, nume,email,datanastere,telefon,photo FROM jsusers`;
...
const create = (user, callback) => {
    const queryString = "INSERT INTO jsusers (nume, prenume, email, datanastere, telefon, photo) VALUES (?, ?, ?, ?, ?, ?)";
    console.log(user);
    db_1.db.query(queryString, [user.nume, user.prenume, user.email, user.datanastere, user.telefon, user.photo], (err, result) => {
        if (err) {
            callback(err);
        }
